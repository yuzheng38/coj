...
const getProblems = function(){
    ...
    // });
    return new Promise((resolve, reject) => {
        ProblemModel.find({}, function(err, returnedProblems) {
            if(err){
                reject(err);
            } else {
                resolve(returnedProblems);
            }
        });
    });
}
...
const getProblem = function(id){
    ...
    // });
        return new Promise((resolve, reject) => {
        ProblemModel.findOne({id: id}, function(err, returnedProblem) {
            if(err){
                reject(err);
            } else {
                resolve(returnedProblem);
            }
        });
    });
}
...
const addProblem = function(newProblem){
    ...
    return new Promise((resolve, reject) => {
        ProblemModel.findOne({name: newProblem.name}, function(err, data) {
            if (data ) {
                reject("Problem name already exists in db");
            } else {
                ProblemModel.count({}, function(err, num) {
                    newProblem.id = num + 1;
                    let newProblemToMongo = new ProblemModel(newProblem);
                    newProblemToMongo.save();
                    resolve(newProblemToMongo);
                });
            }
        });
    });
